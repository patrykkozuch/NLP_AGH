...
from nlp_agh.transformer.attention import MultiHeadAttention
from nlp_agh.transformer.positional_encoding import PositionalEncoding


class Decoder(nn.Module):
    def __init__(self, num_heads: int, d_model: int, d_ff: int, dropout_rate: float, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.ln_masked_attn = nn.LayerNorm(d_model)
        self.masked_attn = MultiHeadAttention(num_heads=num_heads, d_model=d_model, dropout_rate=dropout_rate)
        self.dropout_masked_attn = nn.Dropout(p=dropout_rate)
